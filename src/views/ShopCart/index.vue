      ...
      <div class="cart-body">
        <ul class="cart-list" v-for="(item, index) in cartList" :key="item.id">
          ...
          <li class="cart-list-con5">
            ...
            <input autocomplete="off" type="text" minnum="1" class="itxt" :value="item.skuNum" @change="changeItemCount(item,$event.target.value - item.skuNum,$event)" />
...
import {mapGetters,mapState} from 'vuex'
  export default {
    ...
    computed: {
      ...
      ...mapGetters(['totalCount','totalPrice','isChecked']),
      checkAll:{
        get(){
    ...
    methods: {
      async checkCartItem(item){
          ...
          try {
            await this.$store.dispatch('reqCheckCartItem',{skuId,isChecked})
            await this.$store.dispatch('getShopCartList')
          } catch (error) {
            alert(error.message)
          }
      ...
      async changeItemCount(item,numChange,event){
        ...
        if(num > 0){
          ...
          try {
            await this.$store.dispatch('toAddCart2',{skuId,skuNum})
            await this.$store.dispatch('getShopCartList')
          } catch (error) {
